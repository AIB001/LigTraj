...
####################################
# Version 1.7: Added ligand atoms to MaSIF feature calculation with ligand/protein binary indicator
####################################
import os
import mdtraj as md
import numpy as np
...
def compute_masif_geodesic_embedding(topol, traj_file, sdf, resname="LIG", cutoff=0.4, n_frames=10, distance_mode="euclidean"):
    """
    Compute MaSIF-like embedding for ligand-protein interface patches.
    For euclidean distance: r is distance from pocket center to each atom (protein contact or ligand).
    For geodesic distance: r is total path length from ligand start atom to target atom.
    Now includes both protein contact atoms AND ligand atoms in the feature calculation.
    """
    traj = md.load(traj_file, top=topol)
    if n_frames > traj.n_frames:
        n_frames = traj.n_frames
    ...
    # Get atom-based properties from RDKit
    atom_contribs = {}
    try:
        logp_contribs = Crippen.MolLogP_Contributions(mol)
        for i, contrib in enumerate(logp_contribs):
            atom_contribs[i] = contrib
    except:
        # If Crippen calculation fails, use default value
    ...
    for frame in tqdm(range(n_frames), desc="Computing MaSIF embeddings"):
        ...
        # Now consider both ligand and contacting protein atoms for feature calculation
        all_atoms = np.concatenate([ligand_atoms, contacting_prot_atoms])
        all_coords = np.vstack([t_slice.xyz[0, ligand_atoms, :], t_slice.xyz[0, contacting_prot_atoms, :]])
        center = all_coords.mean(axis=0)

        # Create binary indicator (0 for protein, 1 for ligand)
        is_ligand = np.concatenate([np.ones(len(ligand_atoms)), np.zeros(len(contacting_prot_atoms))])

        ### ---- Compute r ---- ###
        if distance_mode == "euclidean":
            # Simple Euclidean distance from center to each atom
            r = np.linalg.norm(all_coords - center, axis=1)
            ...
            for atom_idx in all_atoms:
                G.add_node(atom_idx, pos=t_slice.xyz[0, atom_idx, :])
            ...
            ### --- 2. Global ligand start atom: closest to center --- ###
            lig_dists = np.linalg.norm(lig_coords - center, axis=1)
            lig_start_atom = ligand_atoms[np.argmin(lig_dists)]

            ### --- 3. Compute r for each atom --- ###
            r = []

            # For all atoms
            for idx, atom_idx in enumerate(all_atoms):
                if is_ligand[idx] == 1:  # Ligand atom
                    # Direct path from start atom to this ligand atom
                    try:
                        distance = nx.dijkstra_path_length(G, lig_start_atom, atom_idx, weight='weight')
                    except nx.NetworkXNoPath:
                        distance = 999.0
                else:  # Protein atom
                    # Find ligand atoms directly contacting this protein atom
                    contact_ligs = []
                    for lig_idx in ligand_atoms:
                        d = np.linalg.norm(t_slice.xyz[0, lig_idx, :] - t_slice.xyz[0, atom_idx, :])
                        if d < cutoff:
                            contact_ligs.append((lig_idx, d))
                    ...
                    try:
                        d1 = nx.dijkstra_path_length(G, lig_start_atom, lig_contact_atom, weight='weight')
                        d2 = nx.dijkstra_path_length(G, lig_contact_atom, atom_idx, weight='weight')
                        distance = d1 + d2
                    except nx.NetworkXNoPath:
                        distance = 999.0
        ...
        ### ---- Compute theta ---- ###
        # Direction vector defined from center to ligand center
        direction = lig_center - center
        direction /= (np.linalg.norm(direction) + 1e-8)
        vectors = all_coords - center
        u_axis = vectors[0] / (np.linalg.norm(vectors[0]) + 1e-8)
        v_axis = np.cross(direction, u_axis)
        v_axis /= (np.linalg.norm(v_axis) + 1e-8)
        ...
        for idx, atom_idx in enumerate(all_atoms):
            # Local shape & curvature (PCA eigenvalues)
            neighbor_coords = []
            for jdx, other_idx in enumerate(all_atoms):
                d = np.linalg.norm(all_coords[idx] - all_coords[jdx])
                if d < 0.5:
                    neighbor_coords.append(all_coords[jdx])
            neighbor_coords = np.array(neighbor_coords)
            ...
            if is_ligand[idx] == 1:  # Ligand atom
                # For ligand atoms, use MDTraj atom properties directly instead of RDKit
                # Find position of this atom in ligand_atoms array
                lig_pos = np.where(ligand_atoms == atom_idx)[0][0]
                
                # Electrostatics for ligand atoms (simplify to avoid RDKit indexing issues)
                atom = t_slice.topology.atom(atom_idx)
                elem = atom.element.symbol
                # Simple heuristic: N, O often have partial charges
                electro = int(elem in ('N', 'O', 'S', 'P', 'F', 'Cl', 'Br', 'I'))
                electro_list.append(electro)
                
                # Hydrophobicity for ligand atoms (using precomputed contributions)
                # Use position in ligand array as simple index (0-based)
                hydro = atom_contribs.get(lig_pos, 0.0)
                hydro_list.append(hydro)
                
                # Hydrogen bond potential for ligand atoms
                hbond = int(elem in ('N', 'O'))
                hbond_list.append(hbond)
                
            else:  # Protein atom
                res = t_slice.topology.atom(atom_idx).residue
                
                # Electrostatics for protein
                if ligand_charge > 0:
                    electro = int(res.name in neg_residues)
                elif ligand_charge < 0:
                    electro = int(res.name in pos_residues)
                ...
                # Hydrophobicity for protein
                hydro = kd_scale.get(res.name, 0.0)
                hydro_list.append(hydro)
                
                # Hydrogen bond potential for protein
                lig_polars = [i for i in ligand_atoms if t_slice.topology.atom(i).element.symbol in ('N', 'O')]
                prot_elem = t_slice.topology.atom(atom_idx).element.symbol
                hbond = 0
                if prot_elem in ('N', 'O'):
                    pairs = [(i, atom_idx) for i in lig_polars]
                    if pairs:
                        dists = md.compute_distances(t_slice, pairs)[0]
                        hbond = int(np.any(dists < 0.35))
                hbond_list.append(hbond)
        ...
        df = pd.DataFrame({
            ...
            'HBonds': hbond_list,
            'Hydrophobicity': hydro_list,
            'IsLigand': is_ligand  # Binary indicator: 0 for protein, 1 for ligand
        })
        df['Frame'] = frame
        all_embeddings.append(df)
...
def _plot_patch_example(df, save_dir, distance_mode):
    """
    Plot polar feature maps for the first frame.
    Each point represents an atom (protein or ligand).
    """
    from matplotlib.colors import LinearSegmentedColormap
    cmap = LinearSegmentedColormap.from_list("custom", ["#00ced1", "#ffa500", "#ff69b4"])

    # Plot all original features plus the new binary IsLigand feature
    features = ["ShapeIndex", "Curvature", "Electrostatics", "HBonds", "Hydrophobicity", "IsLigand"]
    for feat in features:
        fig = plt.figure(figsize=(8, 8))
        ax = plt.subplot(111, polar=True)
        c = df[feat]
